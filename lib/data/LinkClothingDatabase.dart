...
class LinkClothingDatabase {
  static int db_version = 1;

  static Future<Database> makeDatabase() async {
    WidgetsFlutterBinding.ensureInitialized();
    final Future<Database> database = openDatabase(
      join(await getDatabasesPath(), 'linkclothing_database1.db'),
      onCreate: (db, version) {
        return db.execute(
          "CREATE TABLE LinkTable(request_num INTEGER, encoded_img TEXT, img_path TEXT, brand TEXT, detail_url TEXT, fashion_url TEXT, item_url TEXT, name TEXT, price TEXT, score TEXT, category TEXT)",
        );
      },
      version: db_version,
    );
  ...
  static Future<void> insertProduct(ProductClothing p) async {
    ...
    final Database db = await database;
    await db.insert('LinkTable', p.toMap(),
  ...
  static Future<List<ProductClothing>> getRecoResult(int request_num) async {
    ...
    final List<Map<String, dynamic>> maps = await db
        .query('LinkTable', where: 'request_num = ?', whereArgs: [request_num]);
